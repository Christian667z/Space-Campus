...
:: ======================================================
::  Space AI - Installation des dependances
...
"%V_PY%" -m pip install --quiet python-dotenv google-genai google-generativeai groq openai flask flask-cors requests websockets colorama tenacity google-auth google-auth-oauthlib google-auth-httplib2 google-api-python-client anthropic httpx aiohttp pydantic
if !errorlevel! neq 0 (
...
"%V_PY%" -m pip install --quiet SpeechRecognition edge-tts pyttsx3 pyautogui Pillow screeninfo pyperclip tabulate tqdm numpy comtypes
if !errorlevel! neq 0 (
...
if !errorlevel! neq 0 (
    echo [AVERTISSEMENT] PyWebView non installe - Space AI s'ouvrira dans Chrome.
    set /a WARNINGS+=1
) else (
