...
TITLE S.P.A.C.E.A.I - Installation Automatique (www.techenclair.fr)
COLOR 0A
cd /d "%~dp0"

echo ======================================================
echo           INSTALLATION DE S.P.A.C.E.A.I
echo           DEVELOPPEUR : SPACE
echo ======================================================
echo.
...
:: Vérification Racine (présence de main5.py dans le même dossier que le script)
if exist "%~dp0main5.py" (
    set "ROOT_STATUS=OUI"
) else if exist "%~dp0main2.py" (
    set "ROOT_STATUS=OUI"
) else (
    set "ROOT_STATUS=NON"
)
...
if "!ROOT_STATUS!"=="NON" (
    ...
    echo.
    echo POUR CORRIGER :
    echo 1. Allez dans votre dossier Space AI (la ou se trouve main5.py^).
...
if "!CHROME_STATUS!"=="NON" (
    echo [^!] ATTENTION : Chrome est introuvable. Space AI en a besoin.
...
echo.
echo ------------------------------------------------------
echo  Soutenir SPACE : Moncash : +509 46 96 6290    Natcash : +509 35 67 2037
echo ======================================================
...
"%V_PY%" -m pip install python-dotenv google-genai google-generativeai groq openai flask flask-cors requests websockets colorama tenacity google-auth google-auth-oauthlib google-auth-httplib2 google-api-python-client
if %errorlevel% neq 0 (
...
if !errorlevel! neq 0 (
    echo [INFO] PyWebView non installe. Space AI s'ouvrira dans le navigateur Chrome.
)
...
if "!WEBVIEW2_OK!"=="1" (
    ...
    if exist "MicrosoftEdgeWebview2Setup.exe" (
        ...
        echo [INFO] SPACE AI s'ouvrira dans Chrome si WebView2 est absent.
    )
)
...
if !errorlevel! neq 0 (
    ...
    if !errorlevel! neq 0 (
        echo [INFO] Pygame n'a pas pu etre installe. Space AI fonctionnera sans son audio.
    )
)
...
:: --- Controle volume + luminosite (fonctions locales SPACE AI) ---
...
echo [SYSTEME] Generation du fichier de lancement 'DEMARRER_SPACEAI.bat'...
(
echo @echo off
echo TITLE SPACE AI - BY SPACE 
echo COLOR 0B
echo cd /d "%%~dp0"
echo ".\venv\Scripts\python.exe" "main5.py"
echo pause
...
echo ------------------------------------------------------
echo Par defaut, Space AI est configure pour s'adresser a 'Mickael'.
...
echo 3. Lance 'DEMARRER_SPACEAI.bat' pour demarrer ton IA.
echo.
echo ------------------------------------------------------
echo  Soutenir le projet Space AI :
echo  Si tu aimes mon travail, tu peux m'offrir un cafe ici :
      Moncash : +509 46 96 6290    Natcash : +509 35 67 2037
echo ------------------------------------------------------
echo.
echo Aide et support : +509 35 67 2037
echo ======================================================
pause
