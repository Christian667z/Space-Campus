...
SVG_TEMPLATE = """<svg xmlns="http://www.w3.org/2000/svg" width="{size}" height="{size}" viewBox="0 0 24 24" fill="none" stroke="{color}" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
{paths}
</svg>"""
...
ICONS = {
    ...
    "graph": '<polyline points="22 7 13.5 15.5 8.5 10.5 2 17"/><polyline points="16 7 22 7 22 13"/>',
...
# Aliases pour les sections de l'application
ICONS["assistant"] = ICONS["message"]
ICONS["insight"] = ICONS["oracle"]
ICONS["tools"] = ICONS["tool"]
ICONS["library"] = ICONS["book"]
ICONS["flash"] = ICONS["zap"]
ICONS["notes"] = ICONS["file"]
ICONS["stats"] = ICONS["graph"]
ICONS["guide"] = ICONS["book"]
...
class SVGManager:
    ...
    @staticmethod
    def get_svg_path(name: str, color: str = "#ffffff", size: int = 24) -> str:
        """Génère l'icône SVG si elle n'existe pas et retourne le chemin d'accès absolu."""
        if name not in ICONS:
            return ""
            
        SVGManager._ensure_dir()
        
        # Le nom du fichier inclut la couleur (sans #) et la taille pour gérer les variations
        color_hex = color.replace("#", "")
        filename = f"{name}_{color_hex}_{size}.svg"
    ...
    def get_icon(name: str, color: str = "#ffffff", size: int = 24) -> QIcon:
        """Retourne un QIcon prêt à l'emploi."""
        path = SVGManager.get_svg_path(name, color, size)
        if not path:
            return QIcon()
        return QIcon(path)
